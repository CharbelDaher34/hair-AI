...
def get_distinct_field_values(
    ...
    try:
        ...
        index_mapping = mappings.get(index_name, {}).get("mappings", {}).get("properties", {})
...
def es_type_to_pydantic(
    ...
    for field_name, field_props in es_mapping.items():
        ...
        if field_props.get("type") in IGNORED_FIELD_TYPES or field_name in fields_to_ignore:
            continue
        ...
        elif (field_name in category_fields or full_field_path in category_fields) and index_name:
            try:
                ...
                distinct_values = get_distinct_field_values(index_name, field_path, es_host)
...
def example_get_distinct_values():
    ...
    category_types = get_distinct_field_values("user_transactions", "category_type.keyword")
    print(f"Distinct category types: {category_types}")

    # Get distinct transaction types
    transaction_types = get_distinct_field_values("user_transactions", "transaction.type.keyword")
    print(f"Distinct transaction types: {transaction_types}")
    
    # Get distinct receiver names from nested field
    receiver_names = get_distinct_field_values("user_transactions", "transaction.receiver.name.keyword")
    print(f"Distinct receiver names: {receiver_names}")
    
    # Get distinct receiver category types from nested field
    receiver_categories = get_distinct_field_values("user_transactions", "transaction.receiver.category_type.keyword")
...
def extract_model_info(model_class: type[BaseModel], prefix: str = ""):
    ...
    for field_name, field_info in model_class.model_fields.items():
        ...
        # Handle basic types
            ...
            info[full_field_name] = {
                "type": _get_simple_type_name(field_type)
            }
...
class Query(BaseModel):
    ...
    value: Union[str, float, int, List[str], List[date]]  # simplified to str and validated later
