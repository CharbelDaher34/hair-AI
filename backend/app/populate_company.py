...
CANDIDATE_PDF_PATH = "/storage/hussein/matching/ai/app/services/llm/Charbel_Daher_Resume.pdf"

# Industry categories for realistic company data
INDUSTRIES = [
    "Technology", "Healthcare", "Finance", "Education", "Manufacturing",
    "Retail", "Consulting", "Media", "Transportation", "Energy",
    "Real Estate", "Hospitality", "Agriculture", "Construction", "Telecommunications"
]

# HR roles
HR_ROLES = [
    "HR Manager", "Talent Acquisition Specialist", "HR Director", 
    "Recruiter", "HR Business Partner", "People Operations Manager",
    "Talent Manager", "HR Coordinator", "Chief People Officer"
]

# Form field types for company forms
FORM_FIELD_NAMES = [
    "Years of Experience", "Education Level", "Preferred Location",
    "Salary Expectation", "Notice Period", "Availability",
    "Portfolio URL", "LinkedIn Profile", "GitHub Profile",
    "Certifications", "Languages Spoken", "Visa Status"
]
...
JOB_CATEGORIES = [
    ...
    "ux_design", "sales", "marketing", "finance", "operations"
]

JOB_TYPES = ["full_time", "part_time", "contract", "internship"]
EXPERIENCE_LEVELS = ["no_experience", "1-3_years", "3-5_years", "5-7_years", "7-10_years", "10_plus_years"]
SENIORITY_LEVELS = ["entry", "mid", "senior"]
JOB_STATUSES = ["draft", "published"]

# Application statuses
APPLICATION_STATUSES = ["pending", "reviewing", "interviewing", "offer_sent",  "rejected"]
...
def create_company_data() -> Dict:
    ...
    domain_base = company_name.lower().replace(' ', '').replace(',', '').replace('.', '')[:10]
...
def create_form_key_data() -> Dict:
    ...
    if field_type == "select":
        if "Experience" in field_name:
            enum_values = ["0-1 years", "1-3 years", "3-5 years", "5-10 years", "10+ years"]
        ...
        "compensation": {
            ...
            "benefits": fake.random_elements(
                elements=[
                    ...
                    "Paid Time Off", "Stock Options", "Gym Membership",
                    "Professional Development", "Flexible Hours"
                ],
...
def create_company_via_api(company_data: Dict) -> Optional[Dict]:
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            print(f"Error creating company {company_data['name']}: {response.status_code} - {response.text}")
            return None
...
def create_hr_via_api(hr_data: Dict) -> Optional[Dict]:
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            print(f"Error creating HR user {hr_data['email']}: {response.status_code} - {response.text}")
            return None
...
def create_form_key_via_api(form_key_data: Dict, auth_token: str) -> Optional[Dict]:
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            print(f"Error creating form key {form_key_data['name']}: {response.status_code} - {response.text}")
            return None
...
def create_job_via_api(job_data: Dict, auth_token: str) -> Optional[Dict]:
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            print(f"Error creating job {job_data['title']}: {response.status_code} - {response.text}")
            return None
...
def create_candidate_via_api(candidate_data: Dict, auth_token: str) -> Optional[Dict]:
    ...
    try:
        ...
        if not os.path.exists(CANDIDATE_PDF_PATH):
            print(f"Warning: PDF file not found at {CANDIDATE_PDF_PATH}, creating candidate without resume")
            response = requests.post(
        ...
        if response.status_code == 201:
            ...
            print(f"Error creating candidate {candidate_data['full_name']}: {response.status_code} - {response.text}")
            return None
...
def create_application_via_api(application_data: Dict, auth_token: str) -> Optional[Dict]:
...
def populate_hr_users_via_api(companies: List[Dict], hr_per_company: int = 2) -> List[Dict]:
...
def populate_form_keys_via_api(companies: List[Dict], auth_tokens: Dict, keys_per_company: int = 5) -> List[Dict]:
            ...
            while attempts < 10:
                ...
                if form_key_data['name'] not in used_names:
                    ...
                    if form_key:
                        ...
                        print(f"✓ Created form key: {form_key_data['name']} for {company_name}")
                    else:
                        print(f"✗ Failed to create form key: {form_key_data['name']} for {company_name}")
                    break
                attempts += 1
...
def populate_jobs_via_api(companies: List[Dict], auth_tokens: Dict, jobs_per_company: int = 3) -> List[Dict]:
...
def populate_candidates_via_api(companies: List[Dict], auth_tokens: Dict, candidates_per_company: int = 5) -> List[Dict]:
    ...
    for company in companies:
        ...
        for _ in range(num_candidates):
            ...
            if candidate:
                ...
                print(f"✓ Created candidate: {candidate['full_name']} for {company_name}")
            else:
...
def populate_applications_via_api(jobs: List[Dict], candidates: List[Dict], auth_tokens: Dict, applications_per_job: int = 3) -> List[Dict]:
    ...
    for job in jobs:
        ...
        # Skip if no auth token available for this company
        if company_id not in auth_tokens:
            print(f"⚠ No auth token available for {company_name}, skipping applications")
            continue
        ...
        if not company_candidates:
            print(f"⚠ No candidates available for {company_name}, skipping applications for job {job_title}")
            continue
        
        # Create 1-5 applications per job
        num_applications = min(random.randint(1, applications_per_job), len(company_candidates))
        ...
        for candidate in selected_candidates:
            ...
            if application:
                ...
                print(f"✓ Created application: {candidate['full_name']} -> {job_title} ({company_name})")
            else:
                print(f"✗ Failed to create application: {candidate['full_name']} -> {job_title}")
...
def main():
    ...
    try:
        ...
        try:
            num_companies = 1
            hr_per_company = 1
            keys_per_company = int(input("Enter max form keys per company (default 3): ") or "3")
            jobs_per_company = int(input("Enter max jobs per company (default 3): ") or "3")
            candidates_per_company = int(input("Enter max candidates per company (default 4): ") or "4")
            applications_per_job = int(input("Enter max applications per job (default 2): ") or "2")
        except ValueError:
        ...
        # Populate candidates
        candidates = populate_candidates_via_api(companies, auth_tokens, candidates_per_company)
        
        # Populate applications
        applications = populate_applications_via_api(jobs, candidates, auth_tokens, applications_per_job)
