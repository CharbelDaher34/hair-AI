...
@router.post(
    "/", response_model=JobFormKeyConstraintRead, status_code=status.HTTP_201_CREATED
)
def create_job_form_key_constraint(
    *,
    ...
    try:
        constraint = crud_job_form_key_constraint.create_job_form_key_constraint(
...
def read_job_form_key_constraint(
    *, db: Session = Depends(get_session), constraint_id: int
) -> JobFormKeyConstraintRead:
    constraint = crud_job_form_key_constraint.get_job_form_key_constraint(
...
def read_constraints_by_job(
    *, db: Session = Depends(get_session), job_id: int, skip: int = 0, limit: int = 100
) -> List[JobFormKeyConstraintRead]:
    return crud_job_form_key_constraint.get_job_form_key_constraints_by_job(
...
def update_job_form_key_constraint(
    *,
    ...
    if not constraint:
        raise HTTPException(status_code=404, detail="JobFormKeyConstraint not found")
    return crud_job_form_key_constraint.update_job_form_key_constraint(
        db=db, db_constraint=constraint, constraint_in=constraint_in
    )
...
def delete_job_form_key_constraint(
    *, db: Session = Depends(get_session), constraint_id: int
    ...
    constraint = crud_job_form_key_constraint.delete_job_form_key_constraint(
        db=db, constraint_id=constraint_id
    )
    if not constraint:
        raise HTTPException(status_code=404, detail="JobFormKeyConstraint not found")
