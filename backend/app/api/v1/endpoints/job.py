...
def read_jobs_by_status(
    *, db: Session = Depends(get_session), status: str, skip: int = 0, limit: int = 100
) -> List[JobRead]:
...
def update_job(
    *, db: Session = Depends(get_session), job_id: int, job_in: JobUpdate
) -> JobRead:
    job = crud_job.get_job(db=db, job_id=job_id)
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")
...
def delete_job(*, db: Session = Depends(get_session), job_id: int) -> JobRead:
...
def get_form_data(
    *, db: Session = Depends(get_session), job_id: int, request: Request
    ...
    return JobFormData(job=job, form_keys=form_keys, company=company)


@router.get("/public/form-data/{job_id}", response_model=JobFormData)
def get_public_form_data(
    *, db: Session = Depends(get_session), job_id: int
) -> JobFormData:
    ...
    if job.status != "published":
        raise HTTPException(
            status_code=404, detail="Job not available for applications"
        )
    ...
    *, db: Session = Depends(get_session), job_id: int, request: Request
    ...
    try:
        ...
        for match, candidate in match_candidate_pairs:
            ...
            combined_data = {**match.model_dump(), **candidate.model_dump()}
            matches_with_candidates.append(MatchCandidateResponse(**combined_data))

        return MatchResponseWithDetails(matches=matches_with_candidates, job=job)
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Error retrieving matches: {str(e)}"
        )
