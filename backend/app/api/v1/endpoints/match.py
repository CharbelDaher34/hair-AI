...
def read_matches_by_application(*, db: Session = Depends(get_session), application_id: int, skip: int = 0, limit: int = 100) -> List[MatchRead]:
    return crud_match.get_matches_by_application(db=db, application_id=application_id, skip=skip, limit=limit)

@router.patch("/{match_id}", response_model=MatchRead)
def update_match(*, db: Session = Depends(get_session), match_id: int, match_in: MatchUpdate) -> MatchRead:
    if not (match := crud_match.get_match(db=db, match_id=match_id)):
        raise HTTPException(status_code=404, detail="Match not found")
