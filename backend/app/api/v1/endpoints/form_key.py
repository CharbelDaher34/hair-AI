...
def create_form_key(
    *, db: Session = Depends(get_session), form_key_in: FormKeyCreate, request: Request
) -> FormKeyRead:
...
def read_form_key(
    *, db: Session = Depends(get_session), form_key_id: int, request: Request
) -> FormKeyRead:
    current_user: Optional[TokenData] = request.state.user
    if not current_user:
        raise HTTPException(
...
def update_form_key(
    *,
    ...
    return crud_form_key.update_form_key(
        db=db, db_form_key=form_key, form_key_in=form_key_in
    )
