...
@router.post(
    "/", response_model=RecruiterCompanyLinkRead, status_code=status.HTTP_201_CREATED
)
def create_recruiter_company_link(
    *,
    ...
    try:
        link = crud_recruiter_company_link.create_recruiter_company_link(
            db=db, link_in=link_in
        )
...
def read_recruiter_company_link(
    *, db: Session = Depends(get_session), link_id: int, request: Request
) -> RecruiterCompanyLinkRead:
    current_user: Optional[TokenData] = request.state.user
    if not current_user:
        raise HTTPException(
    ...
    link = crud_recruiter_company_link.get_recruiter_company_link(
...
def read_links_by_recruiter(
    *,
    ...
    return crud_recruiter_company_link.get_recruiter_company_links_by_recruiter(
        db=db, recruiter_id=current_user.employer_id, skip=skip, limit=limit
    )


@router.get(
    "/by-target-company/{target_employer_id}",
    response_model=List[RecruiterCompanyLinkRead],
)
def read_links_by_target_company(
    *,
    ...
    return crud_recruiter_company_link.get_recruiter_company_links_by_target_company(
        db=db, target_employer_id=target_employer_id, skip=skip, limit=limit
    )
...
def update_recruiter_company_link(
    *,
    ...
    if not link:
        raise HTTPException(status_code=404, detail="RecruiterCompanyLink not found")
    return crud_recruiter_company_link.update_recruiter_company_link(
        db=db, db_link=link, link_in=link_in
    )


@router.delete("/{link_id}", response_model=RecruiterCompanyLinkRead)
def delete_recruiter_company_link(
    *, db: Session = Depends(get_session), link_id: int
) -> RecruiterCompanyLinkRead:
    link = crud_recruiter_company_link.delete_recruiter_company_link(
        db=db, link_id=link_id
    )
    if not link:
        raise HTTPException(status_code=404, detail="RecruiterCompanyLink not found")
