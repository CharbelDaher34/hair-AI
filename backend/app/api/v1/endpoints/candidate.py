...
def parse_resume_background(candidate_id: int, resume_file_path: str, max_retries: int = 3):
    ...
    for attempt in range(max_retries):
        try:
            print(f"[Background] Starting resume parsing for candidate {candidate_id} (attempt {attempt + 1}/{max_retries})")
            
            # Ensure the resume_file_path is absolute for the parser client
            absolute_resume_file_path = os.path.abspath(resume_file_path)
            print(f"[Background] Absolute resume file path: {absolute_resume_file_path}")
            ...
            if parsed_result is None:
                print(f"[Background] Parsing returned None for candidate {candidate_id} - API may have failed")
            ...
            if isinstance(parsed_result, dict):
                print(f"[Background] Parsed resume result keys for candidate {candidate_id}: {list(parsed_result.keys())}")
            else:
                print(f"[Background] Unexpected result type for candidate {candidate_id}: {type(parsed_result)}")
            ...
            if parsed_result:
                ...
                with Session(engine) as db:
                    ...
                    if candidate:
                        ...
                        print(f"[Background] Successfully updated candidate {candidate_id} with parsed resume data")
                        return  # Success - exit the retry loop
                    else:
                        print(f"[Background] Candidate {candidate_id} not found for resume parsing update")
                        return
            else:
                print(f"[Background] No valid parsing results for candidate {candidate_id} - skipping database update")
                return
                    
        except Exception as parse_err:
            print(f"[Background] Error parsing resume for candidate {candidate_id} (attempt {attempt + 1}): {str(parse_err)}")
    ...
    try:
        ...
        if resume:
            try:
                ...
                background_tasks.add_task(parse_resume_background, candidate.id, permanent_resume_path)
                print(f"[Candidate API] Scheduled background resume parsing for candidate {candidate.id}")
...
def delete_candidate(
    *,
    ...
    try:
        ...
        print(f"[Candidate API] Failed to delete resume file for candidate {candidate_id}: {str(e)}")
...
def get_resume_parsing_status(
    *,
    ...
    has_parsed_data = candidate.parsed_resume is not None and candidate.parsed_resume != {}
