...
def test_query_filtering():
    ...
    test_cases = [
        ...
        ("SELECT * FROM candidate", "SELECT * FROM candidate WHERE candidate.employer_id = 123"),
        ...
        ("SELECT * FROM job WHERE status = 'published'", "SELECT * FROM job WHERE job.employer_id = 123 AND status = 'published'"),
        ("SELECT * FROM candidate WHERE full_name LIKE '%John%'", "SELECT * FROM candidate WHERE candidate.employer_id = 123 AND full_name LIKE '%John%'"),
        
        # Queries that already have employer_id filter (should not be modified)
        ("SELECT * FROM job WHERE employer_id = 456", "SELECT * FROM job WHERE employer_id = 456"),
