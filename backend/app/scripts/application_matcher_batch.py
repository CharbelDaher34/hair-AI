...
def create_match_for_application(application_id: int, max_retries: int = 3) -> bool:
    ...
    for attempt in range(max_retries):
        try:
            print(f"[Matcher] Processing application {application_id} (attempt {attempt + 1}/{max_retries})")
            ...
            with Session(engine) as db:
                ...
                if not candidate or not job:
                    print(f"[Matcher] Missing candidate or job data for application {application_id}")
                    return False
                
                # Check if candidate has parsed resume data
                if not candidate.parsed_resume:
                    print(f"[Matcher] Candidate {candidate.id} has no parsed resume data - skipping")
                    return False
                ...
                if new_match:
                    print(f"[Matcher] Successfully created match {new_match.id} for application {application_id}")
                    print(f"[Matcher] Match status: {new_match.status}")
                    return True
                else:
                    print(f"[Matcher] Failed to create match for application {application_id}")
                    return False
                    
        except Exception as match_err:
            print(f"[Matcher] Error creating match for application {application_id} (attempt {attempt + 1}): {str(match_err)}")
