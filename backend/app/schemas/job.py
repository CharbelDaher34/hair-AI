...
class JobGeneratedData(SQLModel):
    ...
    compensation: compensation_base = Field(
        default=None,
        description="The compensation for the job, example: {'base_salary': 100000, 'benefits': ['Health Insurance', '401(k) Matching', 'Remote Work']}",
    )
    ...
    responsibilities: List[str] = Field(
        default=None,
        description="The responsibilities of the job, example: ['Develop and maintain API endpoints', 'Write clean, maintainable code', 'Collaborate with cross-functional teams', 'Participate in code reviews']",
    )
    skills: skills_base = Field(
        default=None,
        description="The soft and hard skills required for the job, example: {'hard_skills': ['Python', 'SQL', 'AWS'], 'soft_skills': ['Communication', 'Problem Solving', 'Teamwork']}",
    )
    location: Optional[str] = Field(default=None)
    job_category: Optional[str] = Field(
        default=None,
        description="The category of the job, example: 'Software Engineering', 'Data Science', 'Product Management', 'Marketing', 'Sales', 'Finance', 'HR', 'Legal', 'Customer Support', 'Design', 'Other'",
    )
