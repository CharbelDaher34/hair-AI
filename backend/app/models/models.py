...
class Company(CompanyBase, table=True):
    ...
    recruited_to_links: List["RecruiterCompanyLink"] = Relationship(
        ...
        sa_relationship_kwargs={
            "foreign_keys": "RecruiterCompanyLink.target_employer_id"
        },
    )
...
class FormKeyBase(TimeBase):
    ...
    field_type: FieldType = Field(
        ...
        sa_column=Column(
            SQLAlchemyEnum(FieldType, name="fieldtype_enum", create_type=True)
        ),
    )
...
class FormKey(FormKeyBase, table=True):
    ...
    job_constraints: List["JobFormKeyConstraint"] = Relationship(
        back_populates="form_key"
    )
...
class JobBase(TimeBase):
    ...
    experience_level: ExperienceLevel = Field(
        ...
        sa_column=Column(
            SQLAlchemyEnum(
                ExperienceLevel, name="experiencelevel_enum", create_type=True
            )
        ),
    )
    seniority_level: SeniorityLevel = Field(
        ...
        sa_column=Column(
            SQLAlchemyEnum(SeniorityLevel, name="senioritylevel_enum", create_type=True)
        ),
    )
    job_type: JobType = Field(
        default=JobType.FULL_TIME,
        sa_column=Column(
            SQLAlchemyEnum(JobType, name="jobtype_enum", create_type=True)
        ),
    )
    ...
    def validate_compensation_and_skills(cls, v, info):
        if v is not None:
            ...
            if info.field_name == "compensation":
                ...
                if v.get("base_salary") is not None and not isinstance(
                    v["base_salary"], int
                ):
                    raise ValueError(
                        "compensation.base_salary must be an integer or None"
                    )
            ...
            # Validate skills structure
                ...
                if "hard_skills" not in v or "soft_skills" not in v:
                    raise ValueError(
                        "skills must contain hard_skills and soft_skills fields"
                    )
...
class Job(JobBase, table=True):
    ...
    def get_job_data(self):
        ...
        if self.compensation:
            comp_parts = []
            if self.compensation.get("base_salary"):
                comp_parts.append(f"Base Salary: ${self.compensation['base_salary']:,}")
            if self.compensation.get("benefits"):
        ...
        if self.skills:
            skill_parts = []
            if self.skills.get("hard_skills"):
                hard_skills = ", ".join(self.skills["hard_skills"])
                skill_parts.append(f"Technical Skills: {hard_skills}")
            if self.skills.get("soft_skills"):
        ...
        if self.employer:
            company_parts = [f"Company: {self.employer.name}"]
            if self.employer.description:
                company_parts.append(f"Description: {self.employer.description}")
            if self.employer.website:
                company_parts.append(f"Website: {self.employer.website}")
