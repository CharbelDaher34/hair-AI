...
from models.models import HR, Company, Job, Candidate, Application, Match, CandidateEmployerLink
...
def test_data_without_rls():
    ...
    with Session(get_admin_engine()) as session:
        ...
        for link in links:
            print(f"  - Link: Candidate ID {link.candidate_id} -> Employer ID {link.employer_id}")
