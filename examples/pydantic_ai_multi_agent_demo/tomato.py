...
@writing_agent.tool
async def get_research_for_writing(
    ctx: RunContext[SharedDependencies]
) -> str:
...
@writing_agent.tool
async def calculate_word_count(
    ctx: RunContext[SharedDependencies],
    text: str
) -> int:
...
async def demonstrate_agent_capabilities():
    ...
    result1 = await run_multiagent_workflow(
        ...
        subtopics=["Medical Diagnosis", "Drug Discovery", "Patient Care", "Regulatory Challenges"],
    ...
    if result1.research_findings:
        print(f"  - Research Confidence: {result1.research_findings.confidence_score:.2f}")
    if result1.analysis_result:
        print(f"  - Analysis Confidence: {result1.analysis_result.confidence_level:.2f}")
    ...
    result2 = await run_multiagent_workflow(
        ...
        subtopics=["Green Computing", "Circular Economy", "ESG Compliance", "Cost-Benefit Analysis"],
...
async def main():
    ...
    try:
        ...
        print("💡 Make sure you have set up your OpenAI API key in environment variables")
