...
async def main():
    ...
    inputs = ["i love this image so much, so handsome and big muscles", "marina is a beautiful woman", "tomatoooooo is bad"]

    # Schedule the three async calls concurrently
    tasks = [asyncio.create_task(model_inference(data, gemini_agent)) for data in inputs]
