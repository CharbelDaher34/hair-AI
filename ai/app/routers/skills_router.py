...
class CompareTextsRequest(BaseModel):
    text_one: str = Field(
        ..., example="Job requires Python, Machine Learning, and AWS."
    )
    text_two: str = Field(
        ..., example="Candidate proficient in Python, Docker, and Azure."
    )
...
class SkillAnalysisResponse(BaseModel):
    ...
    missing_skills: List[
        str
    ]  # Skills in text_one (e.g. job) but not in text_two (e.g. candidate)
    extra_skills: List[
        str
    ]  # Skills in text_two (e.g. candidate) but not in text_one (e.g. job)
    total_text_one_skills: int = Field(
        alias="total_job_skills"
    )  # Alias for clarity if text_one is job
    total_text_two_skills: int = Field(
        alias="total_candidate_skills"
    )  # Alias for clarity if text_two is candidate
...
async def analyze_skills_endpoint(request: CompareTextsRequest):
    ...
    if not request.text_one.strip() or not request.text_two.strip():
        raise HTTPException(
            status_code=400, detail="Both texts must be provided and cannot be empty."
        )
    try:
...
async def get_skill_resemblance_rate_endpoint(request: CompareTextsRequest):
    ...
    try:
        rate, analysis = skill_ner.calculate_skills_resemblance_rate(
            request.text_one, request.text_two
        )
        # The analysis from calculate_skills_resemblance_rate matches SkillAnalysisResponse structure
        return SkillResemblanceResponse(
            resemblance_rate=rate, skill_analysis=SkillAnalysisResponse(**analysis)
        )
    except Exception as e:
...
async def get_skill_match_details_endpoint(request: CompareTextsRequest):
    ...
    try:
        match_details = skill_ner.get_skill_match_details(
            request.text_one, request.text_two
        )
